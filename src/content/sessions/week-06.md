...
teachers:
  - idris-fenn
tags: ["stage-calculate", "dynamic-pricing"]
buildsOn: [5]
spec:
...
If price can depend on context you don't control, then "the price" stops
being a single number to compare against a reference price (Week 3) or a
history (Week 5) — it becomes a number that might change if you check from a
different device, at a different time, or as a different (or logged-out)
user. A cautious buyer treats a single quoted price as one sample from a
distribution, not as the price.

## The $249 headphones

Hypothetically: a shopper checks the $249 headphones from their phone while
logged in, and again from a friend's laptop in an incognito window, and sees
...
Week 7 stops looking at single transactions and asks what happens when a
price repeats every month, whether or not you notice it.
